...
├── operand_a     SMALLINT
├── operand_b     SMALLINT
├── operand_c     SMALLINT NULL   -- 3項の足し算（Lv7, Lv10）のみ
...
**Question shape by level**

| Levels | Operands | Example |
|--------|----------|---------|
| 1–6, 8–9 | 2 | `23 + 45 = ?` |
| 7, 10 | 3 | `12 + 34 + 56 = ?` |
...
- 算式: 2項 `A + B = ?` または 3項 `A + B + C = ?` — 入力値は `?` 位置にインライン表示
- 算式は **1行で表示**（改行しない）。長い式はフォント縮小や横方向フィットで対応
- テンキー: 電卓配列 + 削除ボタン + 「答える」
- キーボード: 0–9 入力、Backspace / Delete で末尾削除、Enter で送信
- 回答桁数: Lv1–9 は最大3桁（999まで）、Lv10 は最大4桁（2997まで）
...
| 条件 | Lv1 | Lv5 | Lv10 |
|------|-----|-----|------|
| 即答（0秒） | 20点（10+10） | 100点（50+50） | 200点（100+100） |
| 5秒で正解 | 15点 | 75点 | 150点 |
| 10秒以上 | 基本点のみ | 50点 | 100点 |
| 不正解後に正解 | 再回答回数は減点しない（正解時の経過時間で計算） | 同左 | 同左 |
