...
The play screen SHALL allow choosing between **通常モード** (standard 10-question quiz) and **タイムアタック** (time attack). Standard mode behavior SHALL remain unchanged.

#### Scenario: Standard mode unchanged
- **WHEN** a player selects 通常モード
- **THEN** the existing standard quiz flow runs with level selection and 10-question sessions
